...
function sendMessage()
{
	...
	{
		var message = $('#messageBox').prop('value');
		var uid = $('#userName').prop('value');
		indicator_switch(1);
		$.ajax({
			cache: false,
			type: 'POST',
...
function editShout(id) 
{
	...
	text = text.replace(/&gt;/g, ">");
	text = text.replace(/&amp;/g, "&");
	$('#messageBox').prop('value', text).focus();
...
{
	var msg = new String( $('#messageBox').prop('value') );
	...
	if ( $.trim(msg) != '' )
	{
		var id = $('#userId').prop('value');
		$.ajax({
			type: 'POST',
			cache: false,
			dataType: 'json',
			url: 'shoutbox_view.php',
			data: 'last='+g_lastMessageID+'&mode=edit'+'&id='+$('#userName').prop('value')+'&edit_id='+id +'&message='+msg,
			beforeSend: function(xhr) {xhr.setRequestHeader('Shoutbox','shoutbox_js');},
			success: handleReceivingMessages
		});
		...
		for( x = 0; x < data.d.length; x++ ) 
		{
			if($("#sb_msg_" + data.d[x].i).length != 1)
			{
				...
				if ( data.d[x].n == 'Info' )
				{
					...
					$('#wyslij').prop('disabled', 'disabled');
					$('#messageBox').prop('disabled', 'disabled');
				}
				else
				{
...
function wstawianieSB(text, mode)
{
    ...
    if (mode==1)
    {
       	$('#messageBox').focus().prop('value',msg + text);
    }
    else if (mode==0)
    {
        $('#messageBox').focus().prop('value',msg + ' ' + text + ', ');
    ...
    {
        $('#messageBox').focus().prop('value', text);
    }
}
function emotki()
{
